...
the command-line is (usually) a single line; usually [A] "there is one line (DOTADIW)", but there can be "multiple lines"; either [B] "multiple lines, doing the same thing", or [C] "multiple lines, doing different things"...I'm most interested in C (A ∧ B are non-trivial)
